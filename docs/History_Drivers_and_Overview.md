...
Overview of the Data Independence Network (DIN) server-side Trunk and Branch stack and of how it is combined with the Beyond Decentralized (BD) client-side Leaf stack.

## The History of

### Beyond Decentralized
Beyond Decentralized (BD) stack evolved from a small ORM layer meant to run on Google Docs to host family data without placing it on a central server.  As development continued the need for a full blown server running in-side a client browser became clear.  It was needed to accommodate minimal server-side costs of sharing only the Repository transaction logs.
...
1.  Deploying servers in Trunk -> Branch -> Leaf node configuration (with Leaf technology provided by Beyond Decentralized).2.  Utilizing dedicated per-Branch queues for read and write requests.  Utilizing a dedicated per-Trunk queue for cross-Trunk communication.  Continuously combining Trunk and Branch queues for final data verification and on-Trunk persistence.3.  Offloading data synchronization between Leaves to a Leaf-to-Leaf peer-to-peer network, when the sharing strategy for the Repository allows it.
...
The streaming will be done in real time with virtually only network-transition delays (implemented via on-network-card streaming). Because it is split at Trunks and Branches will not have server-bottlenecks (due to network's topological efficiency).
The asynchronous access to the Trunk Queue only needed to validate and record transactions at each Trunk.  The access to the Branch queue by its Leaves is needed when a Leave access a Repository that isn't locally cached and is pre-cached at the Branch due to frequent access (falling back to long-term on-Branch Transaction Log storage and a final fall back to Trunk queue and storage if necessary).   
