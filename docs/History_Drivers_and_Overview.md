...
The idea of making DIN economically feasible manifested itself in a proposed turbase.org platform that would split profits on anonymized profile + originating App UI type advertizing with the network providers and users.  It eventually became clear that any revenue tapping by the framework or any centralization would "defeat the purpose".
